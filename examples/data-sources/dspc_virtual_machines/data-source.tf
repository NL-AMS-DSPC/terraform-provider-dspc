...
output "vm_urns" {
  ...
  value       = [for vm in data.asc_virtual_machines.all.virtual_machines : vm.urn]
}

# Output all VM statuses
output "vm_statuses" {
  description = "Map of virtual machine name to its current status"
  value       = { for vm in data.asc_virtual_machines.all.virtual_machines : vm.name => vm.status }
}

# Output count of VMs
output "vm_count" {
  description = "Total number of virtual machines"
  value       = length(data.asc_virtual_machines.all.virtual_machines)
}
